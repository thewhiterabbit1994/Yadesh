...
const Table = () => {
  const [twoOfCourses, setTwoOfCourses] = useState([])
  const {
  ...
  useEffect(() => {
    for (let i = 0; i < 2; i++) {
      ...
      const arr = [...twoOfCourses, loopResult]
      setTwoOfCourses(arr)
    }
  }, []);
  return (
        ...
        emtyCourse
          ...
          <section className="w-full h-[400px] text-[12px] bg-[#FFF] m-auto ">
            <section className="flex flex-col justify-center items-center w-full h-[400px] leading-6">
              <EmtyLogo />
              <p className="mt-[10px] text-[14px]">لیست دوره های شما خالی است !</p>
              <p className=" text-[#0000005d] mt-[5px]"> برای افزودن دوره جدید روی دکمه زیر کلیک کنید </p>
              <button className="w-[200px] cursor-pointer h-[35px] rounded-lg bg-[#3B8044] mt-[15px] text-[#fff]"> افزودن دوره</button>
            </section>
          </section>
      }
